 ...
 * @author Terry
 */
public class ShowCatalogueShoppingCartAction extends ActionSupport {
    private Logger log=Logger.getLogger(ShowCatalogueShoppingCartAction.class);
    private int page;
    private int rows;
    private String sidx;
    private String sord;
    ...
    public String execute() throws SQLException {
        ...
        if (start >= 0) {
             log.info("start>=0");
            result = productManagementService.getShoppingCart(sci, start, rows);
        } else {
             log.info("else start>=0");
            result = new ArrayList();
        }
        log.info("page="+page+"..total_pages="+total_pages+"..rows="+rows+"..start="+start);
        log.info("size2="+result.size());
        return SUCCESS;
    }
