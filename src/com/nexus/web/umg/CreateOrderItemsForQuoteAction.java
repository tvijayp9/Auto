 ...
 * @author Terry
 */
public class CreateOrderItemsForQuoteAction extends ActionSupport implements ServletRequestAware, ServletContextAware {

    Logger log=Logger.getLogger(CreateOrderItemsForQuoteAction.class);
    ...
    public String execute() throws SQLException {
        ...
        catalogueService.createOrderItemsForQuote(sci, new Integer(id).intValue(), new Integer(supId).intValue(), orderNumber, autoOrderNumber);
        String company = (String) session.get("mycompany");
        String clientname=session.get("loginpage").toString();
        log.info("clientName in.CreateOrderItemsForQuoteAction.."+clientname);
...
//        }
        ...
        //sendOrderMail(email, company, orderNumber, orderDate, emailType);
        sendOrderMail(company, orderNumber, orderDate,supId,id,clientname);
        session.remove("quoteshoppingcart");
        session.remove("quotedeliveryDate");
        session.remove("quotecomment");
        return SUCCESS;
    }
